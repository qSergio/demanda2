...
prueba <- activ[53:208] %>% ts(frequency = 52)
...
df_mlp <- tibble(Actual = test %>% as.vector(), 
                     Predicted = pred$mean, 
...
my_arima <- auto.arima(train,d=1,D=1)
summary(my_arima)
...
my_ensemble_predict <- function(your_df, n_ahead) {
  ...
  train <- your_df[1:end1] %>% ts(frequency = 52)
  test <- your_df[(end1 + 1):N] %>% ts(frequency = 52)
  h <- length(test)
  ...
  ensemble_predicted <- 0.25*(predicted_mlp$mean) + 0.75*(predicted_arima)
  
  return(tibble(Actual = test %>% as.vector(), 
...
# Use the function: 
my_ensemble_predict(prueba, 52) -> df_ensemble #usar prueba
...
bind_rows(df_mlp %>% get_accuracy_measures(), 
          ...
          df_bugs %>% get_accuracy_measures()) %>% 
  mutate(Approach = c("MLP", "ARIMA", "Ensemble", "Modelo Estacional")) %>% 
  select(Approach, everything()) %>% 
  mutate_if(is.numeric, function(x) {round(x, 2)}) %>% 
  knitr::kable()

## Ya contiene Bugs y es el mejor en estas métricas
